...
| 한국어 위키백과 | 개념·배경 지식 요약 (search + extracts, 5분 캐시) | 없음 (User-Agent 명시) |
...
상단 탭으로 **소재 검색** · **웹소설 트렌드**(네이버 시리즈 TOP20 일일 스냅샷 기반: 제목 키워드 빈도·랭킹 상승·신규 진입) · **조선인물고**(깃허브 페이지에 배포된 조선 시대 인물·사건·지리 열람 뷰어, iframe 연결)를 전환할 수 있습니다.
...
4. 장르 선택 (판타지/로맨스/무협/현대/미스터리/재난/SF/소설 작법/마스터 플롯) → 소재 선택 (108개) — **마스터 플롯**은 토비어스 《작가를 위한 20가지 플롯》 기반으로 탐구·모험·추격·구출·탈출·복수·수수께끼·라이벌·약자의 역전·유혹·변신·전환·성숙·사랑·금지된 사랑·희생·발견·탐닉의 과실·승천·추락을 소재화
5. **검색 결과를 KCI 보도자료 / KCI 논문 / 위키백과 / Google뉴스 그룹으로 분리 표시** — KCI 결과가 뉴스에 묻히지 않고 항상 먼저 나옵니다. 어떤 소재 검색어로 매칭됐는지 태그로 표시
...
웹소설 랭킹 트렌드 리포트 (네이버 시리즈 TOP20 일일 스냅샷 기반)

| 파라미터 | 타입 | 기본값 | 설명 |
|---|---|---|---|
| `days` | int | 7 | 분석에 사용할 최근 스냅샷 일수 (최대 30) |

- 반환: `latest_date`/`prev_date`(비교일) · `keywords`(제목 키워드 빈도) · `risers`(랭킹 상승, ▲ 델타순) · `fallers`(하락) · `new_entries`(신규 진입) · `latest_titles`(금일 TOP 목록)
- 스냅샷이 1일치뿐이면 `risers`/`fallers`/`new_entries` 비교가 비어 있습니다 (다음 날 수집부터 채워짐)
