...
async function initApp() {
  ...
  window.editor = { 
    ...
    toggleSplitMode, loadSplitRefFile, toggleTypewriterMode, createBackup, 
  ...
  window.graph = { 
    ...
    selectNode, updateNode, updateNodeType, updateNodeColor, updateNodeImage, uploadNodeImage, saveNodeWiki, 
  ...
  // window.typeMusic는 type-music.js에서 직접 설정됨
...
// ============================================================
// EXPORT CRITIQUE AS IMAGE
// ============================================================

async function exportCritiqueAsImage(toClipboard) {
  const container = document.getElementById('critiqueResult');
  if (!container || !container.classList.contains('show')) {
    showToast('?�️ ?�시?�중?� 비평 결과가 ?�습?�다.');
    return;
  }

  const scale = parseFloat(document.getElementById('critiquePngScale')?.value) || 2;
  const maskScores = document.getElementById('critiqueMaskScores')?.checked;
  const maskRadar = document.getElementById('critiqueMaskRadar')?.checked;
  const maskSummary = document.getElementById('critiqueMaskSummary')?.checked;
  const maskSentences = document.getElementById('critiqueMaskSentences')?.checked;
  const maskDetailed = document.getElementById('critiqueMaskDetailed')?.checked;

  const elementsToHide = [];
  
  if (maskScores) {
    elementsToHide.push(...container.querySelectorAll('.score-summary, .score-bars'));
  }
  if (maskRadar) {
    elementsToHide.push(...container.querySelectorAll('.radar-wrap'));
  }
  if (maskSummary) {
    elementsToHide.push(...container.querySelectorAll('.critique-section:has(h4:contains("총평")), .critique-section:has(h4:contains("핵심 분석"))'));
    // Fallback for :has() not supported
    const sections = container.querySelectorAll('.critique-section');
    sections.forEach(section => {
      const h4 = section.querySelector('h4');
      if (h4 && (h4.textContent.includes('총평') || h4.textContent.includes('핵심 분석'))) {
  ...
  if (maskSentences) {
    ...
    sections.forEach(section => {
      const h4 = section.querySelector('h4');
      if (h4 && (h4.textContent.includes('최강') || h4.textContent.includes('최약'))) {
        elementsToHide.push(section);
      }
    });
  }
  if (maskDetailed) {
    elementsToHide.push(...container.querySelectorAll('.critique-section.prof'));
  }
  ...
  try {
    const canvas = await html2canvas(container, {
      ...
      useCORS: true,
      logging: false,
      backgroundColor: getComputedStyle(document.body).getPropertyValue('--panel2').trim() || '#1e222c',
    ...
    if (toClipboard) {
      canvas.toBlob(async (blob) => {
        try {
          ...
          document.getElementById('critiqueExportToast').textContent = '?�� ?�립보드??복사?�었?�니?? (' + scale + 'x)';
          document.getElementById('critiqueExportToast').style.display = 'block';
          setTimeout(() => { document.getElementById('critiqueExportToast').style.display = 'none'; }, 3000);
          showToast('?�� ?�립보드??복사 ?�료 (' + scale + 'x)');
        } catch (e) {
          showToast('?�️ ?�립보드 복사 ?�패: ' + e.message);
        }
      }, 'image/png');
    } else {
      const link = document.createElement('a');
      const dateStr = new Date().toISOString().slice(0, 10).replace(/-/g, '');
      link.download = `critique_${dateStr}_${scale}x.png`;
      link.href = canvas.toDataURL('image/png');
      link.click();
      document.getElementById('critiqueExportToast').textContent = '?�� PNG ?�???�료 (' + scale + 'x)';
      document.getElementById('critiqueExportToast').style.display = 'block';
      setTimeout(() => { document.getElementById('critiqueExportToast').style.display = 'none'; }, 3000);
      showToast('?�� PNG ?�???�료 (' + scale + 'x)');
    }
  } catch (e) {
    elementsToHide.forEach(el => {
      if (el && originalStyles.has(el)) {
