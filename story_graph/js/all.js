...
function playPianoNote(freq, vol, duration, preset) {
  const ctx = getAudioContext();
...
function playKeyboardSound(key, vol) {
  const ctx = getAudioContext();
  const now = ctx.currentTime;
...
function playImprovNote(key, vol) {
  const ctx = getAudioContext();
  const now = ctx.currentTime;
...
async function initSoundfont(instrument) {
...
function playNote(noteNumber, volume, options) {
...
function getCurrentInstrumentName() {
...
function loadAudioBuffer(arrayBuffer, onSuccess, onError) {
  const ctx = getAudioContext();
  ctx.decodeAudioData(
    arrayBuffer, 
    function(buf) {
...
function playSegment(audioBuffer, index, volume, segmentLength, timeoutSec) {
...
function getSegmentCount(audioBuffer, segmentLength) {
  if (!audioBuffer) return 0;
  const len = segmentLength || 2;
  return Math.floor(audioBuffer.duration / len);
}

function getDuration(audioBuffer) {
  if (!audioBuffer) return 0;
...
function updateNodeImage(name, url) {
...
function toggleWikiPanel() {
  ...
  if (window.cy) setTimeout(function() { window.cy.resize(); window.cy.fit(null, 50); }, 100);
}
...
function toggleNodeOverlay(nid) {
...
function exportGraphToClipboard() {
  ...
  for (const [name, n] of state.nodes) {
    data.entities.push({ name: n.name, type: n.type, description: n.desc, image: n.image || '' });
  }
...
// ============================================================
// TYPE MUSIC - 키보드 스위치 소리 + 배경음악 + 볼륨 덕킹
// ============================================================
...
function initTypeMusic() {
  try {
    ...
    tmState.audioCtx = ctx;
    var g = ctx.createGain();
    g.gain.value = tmState.fxVol;
    g.connect(ctx.destination);
    tmState.fxGain = g;
  } catch(e) {}
}
...
function toggleTypeMusic() {
  tmState.active = !tmState.active;
  var panel = document.getElementById('typeMusicPanel');
  var btn = document.getElementById('typeMusicBtn');
  if (panel) panel.style.display = tmState.active ? 'block' : 'none';
  if (btn) btn.style.borderColor = tmState.active ? 'var(--c-loc)' : '';
  if (!tmState.active) stopBgMusic();
  return tmState.active;
}

function togglePlay() {
  if (!tmState.bgAudio) return;
  var btn = document.getElementById('tmPlayBtn');
  if (tmState.isPlaying) {
    tmState.bgAudio.pause();
    tmState.isPlaying = false;
    if (btn) btn.textContent = '▶ 재생';
  } else {
    tmState.bgAudio.play().catch(function(){});
    tmState.isPlaying = true;
    if (btn) btn.textContent = '⏸ 일시정지';
    tmState.lastKeyTime = Date.now();
    startDuckTimer();
  }
}
...
function clearDuckTimer() {
  if (tmState.duckTimer) {
    clearInterval(tmState.duckTimer);
    tmState.duckTimer = null;
  }
}

function startDuckTimer() {
  clearDuckTimer();
  tmState.duckTimer = setInterval(function() {
    var idle = Date.now() - tmState.lastKeyTime;
    if (tmState.speedSync && tmState.bgAudio && tmState.isPlaying) {
      var cur = tmState.bgAudio.playbackRate;
      if (idle > 800 && cur > 1) {
        tmState.bgAudio.playbackRate = Math.max(1, cur - 0.03);
      } else if (idle > 2000 && cur < 1) {
        tmState.bgAudio.playbackRate = Math.min(1, cur + 0.02);
      }
    }
    if (idle > 600) {
      tmState.duckLevel = Math.max(0, tmState.duckLevel - 0.04);
      updateMusicVol();
      if (tmState.duckLevel <= 0) {
        clearDuckTimer();
        if (tmState.bgAudio && tmState.isPlaying) {
          tmState.bgAudio.pause();
          tmState.isPlaying = false;
          var btn = document.getElementById('tmPlayBtn');
          if (btn) btn.textContent = '▶ 재생';
...
function updateMusicVol() {
  var vol = tmState.musicVol * tmState.duckLevel;
  if (tmState.bgAudio) tmState.bgAudio.volume = vol;
}

function handleTypeKey(key) {
  if (!tmState.active) return;
  var skip = ['F1','F2','F3','F4','F5','F6','F7','F8','F9','F10','F11','F12',
  ...
  if (tmState.keySoundEnabled) playSwitchSound();
  var now = Date.now();
  tmState.lastKeyTime = now;
  tmState.keyTimes.push(now);
  if (tmState.keyTimes.length > 40) tmState.keyTimes.shift();
  var recent = tmState.keyTimes.filter(function(t) { return now - t < 3000; });
  var rpmEl = document.getElementById('tmRpmDisplay');
  if (recent.length >= 2) {
    var span = recent[recent.length - 1] - recent[0];
    var rpm = span > 0 ? Math.round(recent.length / span * 60000) : 0;
    if (rpmEl) rpmEl.textContent = Math.min(rpm, 999) + ' RPM';
  } else if (rpmEl) {
  ...
  if (tmState.speedSync && tmState.bgAudio && tmState.isPlaying) {
    if (recent.length >= 3) {
      var intervals = [];
      for (var i = 1; i < recent.length; i++) intervals.push(recent[i] - recent[i-1]);
      var avg = intervals.reduce(function(a,b){return a+b;}, 0) / intervals.length;
      var rate = Math.max(0.4, Math.min(2.0, Math.pow(60000 / avg / 200, 0.45)));
      tmState.bgAudio.playbackRate = rate;
    }
  }
  if (tmState.duckLevel < 1) {
    tmState.duckLevel = 1;
    updateMusicVol();
    if (tmState.bgAudio && !tmState.isPlaying && tmState.bgAudio.src) {
      tmState.bgAudio.play().catch(function(){});
      tmState.isPlaying = true;
      var btn = document.getElementById('tmPlayBtn');
      if (btn) btn.textContent = '⏸ 일시정지';
  ...
  if (tmState.isPlaying && !tmState.duckTimer) startDuckTimer();
}

function playSwitchSound() {
  if (!tmState.audioCtx || !tmState.active) return;
  var ctx = tmState.audioCtx;
  if (ctx.state === 'suspended') ctx.resume();
  var vol = tmState.fxVol;
  var type = tmState.switchType;
  if (type === 'custom' && tmState.customSoundBuffer) {
    var src = ctx.createBufferSource();
    src.buffer = tmState.customSoundBuffer;
    var g = ctx.createGain();
    g.gain.value = vol;
    src.connect(g);
    g.connect(tmState.fxGain);
    src.start();
    return;
  }
  ...
  osc.connect(gain);
  gain.connect(tmState.fxGain);
  var freq = 800, oscType = 'square';
  if (type === 'blue') { freq = 1200; oscType = 'square'; }
  else if (type === 'brown') { freq = 900; oscType = 'triangle'; }
  else if (type === 'red') { freq = 700; oscType = 'sine'; }
  else if (type === 'black') { freq = 500; oscType = 'sawtooth'; }
  else if (type === 'pebble') { freq = 1100; oscType = 'triangle'; }
  else if (type === 'mungge') { freq = 600; oscType = 'sine'; }
  osc.type = oscType;
  osc.frequency.setValueAtTime(freq, now);
  osc.start(now);
  osc.stop(now + 0.05);
  if (type === 'blue' || type === 'pebble') {
...
function loadAudioFile(file) {
  stopBgMusic();
  if (tmState.bgAudio) {
    if (tmState.bgAudio._url) URL.revokeObjectURL(tmState.bgAudio._url);
    tmState.bgAudio.src = '';
  }
  var url = URL.createObjectURL(file);
  var audio = new Audio(url);
  audio.loop = true;
  audio.volume = tmState.musicVol;
  audio._url = url;
  tmState.bgAudio = audio;
  var nameEl = document.getElementById('tmFileName');
...
function loadKeySound(file) {
  if (!tmState.audioCtx) return;
  var reader = new FileReader();
  reader.onload = function(e) {
    tmState.audioCtx.decodeAudioData(e.target.result).then(function(buf) {
      tmState.customSoundBuffer = buf;
      var lbl = document.getElementById('tmKeySoundLabel');
      if (lbl) lbl.style.opacity = '0.6';
      setTimeout(function(){ if (lbl) lbl.style.opacity = '1'; }, 200);
    }).catch(function(){});
  };
...
function bindTypeMusicUI() {
  var sw = document.getElementById('tmSwitchType');
  if (sw) sw.addEventListener('change', function(e) {
    tmState.switchType = e.target.value;
    var lbl = document.getElementById('tmKeySoundLabel');
    if (lbl) lbl.style.display = e.target.value === 'custom' ? 'inline' : 'none';
  });
  var ks = document.getElementById('tmKeySoundOn');
  if (ks) ks.addEventListener('change', function(e) { tmState.keySoundEnabled = e.target.checked; });
  var kf = document.getElementById('tmKeySoundFile');
  if (kf) kf.addEventListener('change', function(e) { if (e.target.files[0]) loadKeySound(e.target.files[0]); });
  var fi = document.getElementById('tmAudioFile');
  if (fi) fi.addEventListener('change', function(e) { if (e.target.files[0]) loadAudioFile(e.target.files[0]); });
  var fx = document.getElementById('tmFxVol');
  if (fx) fx.addEventListener('input', function(e) {
    tmState.fxVol = parseFloat(e.target.value);
    var lbl = document.getElementById('tmFxVolLabel');
    if (lbl) lbl.textContent = Math.round(tmState.fxVol * 100) + '%';
  });
  var mv = document.getElementById('tmMusicVol');
  if (mv) mv.addEventListener('input', function(e) {
    tmState.musicVol = parseFloat(e.target.value);
    updateMusicVol();
    var lbl = document.getElementById('tmMusicVolLabel');
    if (lbl) lbl.textContent = Math.round(tmState.musicVol * 100) + '%';
  });
  var ss = document.getElementById('tmSpeedSync');
  if (ss) ss.addEventListener('change', function(e) {
    tmState.speedSync = e.target.checked;
    if (tmState.bgAudio) tmState.bgAudio.playbackRate = 1.0;
  });
  document.getElementById('typeMusicBtn')?.addEventListener('click', toggleTypeMusic);
}
...
window.toggleTypeMusic = toggleTypeMusic;
...
async function initApp() {
  ...
  window.editor = { execCmd, saveCurrentFile, saveAsFile, createNewDocument, refreshFileList, openFile, deleteFile, zoomEditor, zoomReset, closeFindInline, doFindInDoc, doFindNext, doFindPrev, toggleSplitMode, loadSplitRefFile, toggleTypewriterMode, createBackup, showSnapshotDialog, closeSnapshotDialog, saveSnapshot, showDocStats, setupAutoSave, toggleEditorTts, readEditorText, stopEditorTts, showFindReplace, closeFindReplace, findInAllFiles, replaceInAllFiles, showMergeSplit, closeMergeSplit, showMergeView, showSplitView, executeMerge, executeSplit, toggleMobilePreview, closeMobilePreview, updateMobilePreview, hasGraphFile };
  window.graph = { initCytoscape, syncCytoscapeFromNodes, applyFilters, toggleFilter, updateGraphCounts, selectNode, updateNodeType, updateNodeColor, updateNodeImage, saveNodeWiki, removeNode, toggleBreathe, toggleSpring, runGraphLayout, zoomGraph, resetGraphView, exportPNG, openGraphFloating, toggleWikiPanel,closeGraphFloating, updateNodeOverlays, toggleNodeOverlay, cycleWikiMode, loadGraphForCurrentFile, exportGraphToClipboard, populateVoiceSelects, onTtsProviderChange, readWikiText, stopReading, addVoiceMapping, removeVoiceMapping };
  window.modules = { copyWithPrompt, copyCritiquePrompt, autoFixJSON, applyGraphData, processPastedResult, autoFixAndProcess, clearResultArea, processCritiqueResult, renderCritiqueResult, drawRadarChart };
  // window.typeMusic는 all.js 내에서 직접 설정됨
